...
#define REG_PAYLOAD_LENGTH_6X     0x0702 // https://github.com/beegee-tokyo/SX126x-Arduino/blob/master/src/radio/lr1121/lr1121.h#L98
...
lr1121 *lr1121::instances[2] = {NULL, NULL};

lr1121::lr1121(int ss, int busy, int instance_id) :
  ...
  _onReceive(NULL)
{
  instances[_instance_id] = this; setTimeout(0); }
...
uint8_t ISR_VECT lr1121::readRegister(uint16_t address) {
  return singleTransfer(OP_READ_REGISTER_6X, address, 0x00);
}

void lr1121::writeRegister(uint16_t address, uint8_t value) {
  singleTransfer(OP_WRITE_REGISTER_6X, address, value);
}

uint8_t ISR_VECT lr1121::singleTransfer(uint8_t opcode, uint16_t address, uint8_t value) {
  waitOnBusy();
...
void lr1121::executeOpcode(uint8_t opcode, uint8_t *buffer, uint8_t size) {
  waitOnBusy();
  digitalWrite(_ss, LOW);
  SPI.beginTransaction(_spiSettings);
  SPI.transfer(opcode);
...
void lr1121::executeOpcodeRead(uint8_t opcode, uint8_t *buffer, uint8_t size) {
  waitOnBusy();
  digitalWrite(_ss, LOW);
  SPI.beginTransaction(_spiSettings);
  SPI.transfer(opcode);
...
void lr1121::onReceive(void(*callback)(int)){
  ...
  if (callback) {
    ...
    executeOpcode(OP_SET_IRQ_FLAGS_6X, buf, 8);
      ...
      #endif
    #endif
    attachInterrupt(digitalPinToInterrupt(_dio0), lr1121::onDio0Rise_0, RISING);
...
void lr1121::sleep() { uint8_t byte = 0x00; executeOpcode(OP_SLEEP_6X, &byte, 1); }
...
void lr1121::handleDio0IfPending() {
  if (_dio0_pending) {
    _dio0_pending = false;
    handleDio0Rise();
  }
...
void ISR_VECT lr1121::handleDio0Rise() {
  uint8_t buf[2];
  buf[0] = 0x00;
  buf[1] = 0x00;
  executeOpcodeRead(OP_GET_IRQ_STATUS_6X, buf, 2);
...
void lr1121::onDio0Rise_0() {
  if (instances[0] && instances[0]->_onReceive) {
    instances[0]->_dio0_pending = true;
  }
}

void lr1121::onDio0Rise_1() {
  if (instances[1] && instances[1]->_onReceive) {
    instances[1]->_dio0_pending = true;
  }
}
...
void lr1121::SetPaConfig() {
    bool isSubGHz = _isSubGHz;
    uint8_t Pabuf[4];
    if (isSubGHz) {
