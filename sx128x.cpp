...
sx128x::sx128x()
    : _spiSettings(8E6, MSBFIRST, SPI_MODE0), _ss(LORA_DEFAULT_SS_PIN),
      ...
      _rxen(pin_rxen), _busy(LORA_DEFAULT_BUSY_PIN), _txen(pin_txen),
      _frequency(0), _txp(0), _sf(0x05), _bw(0x34), _cr(0x01), _packetIndex(0),
      _implicitHeaderMode(0), _payloadLength(255), _crcMode(0),
      _fifo_tx_addr_ptr(0), _fifo_rx_addr_ptr(0), _rxPacketLength(0),
      _preinit_done(false), _tcxo(false) {
  setTimeout(0);
}
...
void ISR_VECT sx128x::onDio0Rise() {
  ...
  // Therefore, the modem is set into receive mode each time a packet is
  // received.
  if (sx128x_modem.getPacketValidity()) {
...
void sx128x::handleDio0Rise() {
  ...
  // If implicit header mode is enabled, use pre-set packet length as payload
  // length instead. See SX1280 datasheet v3.2, page 92
  ...
  if (_receive_callback) {
    _receive_callback(_rxPacketLength);
  }
}
...
// TODO: Check if this change causes issues on any platforms
#if MCU_VARIANT == MCU_ESP32
#if BOARD_MODEL == BOARD_T3S3 || BOARD_MODEL == BOARD_HELTEC32_V3 ||           \
    BOARD_MODEL == BOARD_HELTEC32_V4 || BOARD_MODEL == BOARD_TDECK
  ...
  while (((millis() - start) < 500) && (millis() >= start)) {
    ...
    if ((version_msb == 0xB7 && version_lsb == 0xA9) ||
        (version_msb == 0xB5 && version_lsb == 0xA9)) {
      break;
    }
    delay(100);
  }
...
uint8_t ISR_VECT sx128x::singleTransfer(uint8_t opcode, uint16_t address,
                                        uint8_t value) {
  waitOnBusy();
  uint8_t response;
...
void sx128x::executeOpcode(uint8_t opcode, uint8_t *buffer, uint8_t size) {
  ...
  if (size > 0) {
    ...
    while (offset < size) {
      size_t chunk =
          (size - offset > sizeof(temp)) ? sizeof(temp) : (size - offset);
...
void sx128x::setPacketParams(uint32_t target_preamble_symbols,
                             uint8_t headermode, uint8_t payload_length,
                             uint8_t crc) {
  if (last_me_result_target != target_preamble_symbols) {
    // Calculate exponent and mantissa values for modem
    if (target_preamble_symbols >= 0xF000)
      target_preamble_symbols = 0xF000;
    ...
    while (e <= 15) {
      while (m <= 15) {
        ...
        if (calculated_preamble_symbols >= target_preamble_symbols - 4)
          break;
        m++;
      }
...
int sx128x::begin(unsigned long frequency) {
  ...
  setPacketParams(_preambleLength, _implicitHeaderMode, _payloadLength,
                  _crcMode);
  setTxPower(_txp);
...
bool sx128x::dcd() {
  ...
  if ((buf[0] & IRQ_PREAMBLE_DET_MASK_8X) != 0) {
    carrier_detected = true;
    if (preamble_detected_at == 0) {
      preamble_detected_at = now;
    }
    if (now - preamble_detected_at >
        lora_preamble_time_ms + lora_header_time_ms) {
...
int ISR_VECT sx128x::read() {
  ...
  if (_packetIndex == 0) {
    ...
    // If implicit header mode is enabled, read packet length as payload length
    // instead. See SX1280 datasheet v3.2, page 92
    if (_implicitHeaderMode == 0x80) {
      size = _payloadLength;
    } else {
      size = rxbuf[0];
...
void sx128x::setPins(int ss, int reset, int dio0, int busy, int rxen,
                     int txen) {
  _ss = ss;
  _reset = reset;
  _dio0 = dio0;
...
void sx128x::setPreambleLength(long preamble_symbols) {
  setPacketParams(preamble_symbols, _implicitHeaderMode, _payloadLength,
                  _crcMode);
}
