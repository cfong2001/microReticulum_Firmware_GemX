...
#define OP_SET_IRQ_FLAGS_6X                                                    \
  0x08 // Also provides info such as
...
#define REG_PAYLOAD_LENGTH_6X                                                  \
  0x0702 // https://github.com/beegee-tokyo/SX126x-Arduino/blob/master/src/radio/sx126x/sx126x.h#L98
...
sx126x::sx126x()
    : _spiSettings(16E6, MSBFIRST, SPI_MODE0), _ss(LORA_DEFAULT_SS_PIN),
      _reset(LORA_DEFAULT_RESET_PIN), _dio0(LORA_DEFAULT_DIO0_PIN),
      _busy(LORA_DEFAULT_BUSY_PIN), _rxen(LORA_DEFAULT_RXEN_PIN), _frequency(0),
      _txp(0), _sf(0x07), _bw(0x04), _cr(0x01), _ldro(0x00), _packetIndex(0),
      _preambleLength(18), _implicitHeaderMode(0), _payloadLength(255),
      _crcMode(1), _fifo_tx_addr_ptr(0), _fifo_rx_addr_ptr(0), _packet({0}),
      _preinit_done(false), _onReceive(NULL) {
  setTimeout(0);
}
...
bool sx126x::preInit() {
  ...
  while (((millis() - start) < 2000) && (millis() >= start)) {
    ...
    if (uint16_t(syncmsb << 8 | synclsb) == 0x1424 ||
        uint16_t(syncmsb << 8 | synclsb) == 0x4434) {
      break;
    }
    delay(100);
  }
...
uint8_t ISR_VECT sx126x::singleTransfer(uint8_t opcode, uint16_t address,
                                        uint8_t value) {
  waitOnBusy();
...
void sx126x::setPacketParams(long preamble_symbols, uint8_t headermode,
                             uint8_t payload_length, uint8_t crc) {
  // Because there is no access to these registers on the sx1262, we have
  // to set all these parameters at once or not at all.
  uint8_t buf[9];
...
void sx126x::calibrate_image(long frequency) {
  ...
  if (frequency >= 430E6 && frequency <= 440E6) {
    ...
    image_freq[0] = 0xE1;
    image_freq[1] = 0xE9;
  } // TODO: Allow higher freq calibration
...
void sx126x::setTxPower(int level, int outputPin) {
  ...
  // DS_SX1261-2_V1.2 datasheet chapter 15.2 RegTxClampConfig = @address 0x08D8
  writeRegister(0x08D8, readRegister(0x08D8) | (0x0F << 1));

  uint8_t pa_buf[4];
  pa_buf[0] = 0x04; // PADutyCycle needs to be 0x04 to achieve 22dBm output, but
                    // can be lowered for better efficiency at lower outputs
